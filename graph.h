...
template <typename T>
class graph
{ // Class of unweighted undirected graph
public:
    ...
    void remove_node(T u)
    {
        if (adj.find(u) != adj.end())
        {
            ...
            for (auto i : adj[u])
            { // removing the given vertex from the adjacency list of its neighbours
    ...
    {
        if (adj.find(u) != adj.end())
        { // if vertex u is present in the graph
            auto it = find(adj[u].begin(), adj[u].end(), v);
            if (it != adj[u].end())
            {                                                        // if vertex v is also present in the graph
    ...
    std::vector<T> bfs(T s)
    {
        ...
        while (!q.empty())
        {
            ...
            for (auto u : adj[v])
            { // pushing all the non-visited neighbours of the popped vertex in the queue
                if (!vis[u])
                {
                    q.push(u);
    ...
    std::vector<T> dfs(T u)
    {
        ...
        while (!s.empty())
        {
            ...
            for (auto u : adj[v])
            { // pushing all the non-visited neighbours of the popped vertex in the stack
                if (!vis[u])
                {
                    s.push(u);
                    vis[u] = true; // and marking them as visited
...
public:
    ...
    std::vector<std::vector<T>> SCCs()
    {
        ...
        std::map<T, bool> vis;                   // vis map marks visited vertices as true
        for (auto it : adj)
        { // Calling dfs on first key in adj
            if (!vis[it.first])
            {
        ...
        while (!st.empty())
        { // Popping the vertex from top one by one
            int node = st.top();
            st.pop();
            if (!vis[node])
            { // and then calling dfs on the transpose graph
                std::vector<T> SCC;
...
template <typename T>
class wgraph
{ // Class of weighted undirected graph
public:
    std::map<T, std::vector<std::pair<T, int>>> adj; // adj is the adjacency list of the graph
    int number_of_edges = 0;                         // number of edges stores the total edges in the graph

    void add_edge(T v, T w, int k = 1)
    {                             // This function adds an edge between given two vertices of given weight assuming default edge weight as 1.
...
template <typename T>
class wdigraph
{ // Class of weighted directed graph
public:
